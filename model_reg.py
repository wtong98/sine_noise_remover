...
def _build_loss(inputs, params):
    ...
    mse_list = [tf.losses.mean_squared_error(labels=tf.reshape(inputs[i], [-1]), 
                                             ...
                                             + _bound_loss(frequency_list[i], (1., 3.))
                        for i in range(batch_size)]
...
def _bound_loss(freq, interval):
    difference = tf.sign(freq - interval[0]) + tf.sign(freq-interval[1])
    bound_loss = difference ** 2 * 999
...
def train(sess, waveform, iterations=2000, log_every=50, save_every=100):
    ...
    for step in range(iterations):
        is_training = True
        sess.run(train_op, feed_dict={inputs: waveform})
        if step % log_every == 0:
...
if __name__ == "__main__":
    ...
    test_waveform = np.reshape(test_waveform, (-1, time * s_rate, 1))
    with tf.Session() as sess:
        sess.run(tf.global_variables_initializer())
        train(sess, waveform=test_waveform, iterations=10, log_every=1, save_every=20)
